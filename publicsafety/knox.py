...
class Knox(Spatial_Temporal_Test):
  ...
  def compute_test_statistic(self, t1_as_leading_indicator=False, dist_scale_feet=None, time_scale_days=None, verbose=True, nrand=1000): 
    ...
    for d1i in range(self.N1):
      ...
      close_in_space_time = self.get_time_proximate_points(self.t1[d1i], time_scale_days, t1_as_leading_indicator, close_in_space)
      ...
      #TODO: Need to determine if they asked for a one-sided vs two sided test (and if had to switch t1 and t2)
      for i in range(nrand):
        close_in_space_time = self.get_time_proximate_points(randtimes[i][d1i], time_scale_days, t1_as_leading_indicator, close_in_space)
