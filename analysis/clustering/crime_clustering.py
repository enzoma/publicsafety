...
f = open("normalized_crime_by_tract_per_thousand.csv", 'rb')
...
# Decided to use 9 clusters
 clusterer = KMeans(init="k-means++", n_clusters = 9, n_init = 10)
 clusterer.fit(new_data)
 clusterer.predict(new_data) #shows cluster assignment for each tract

a = clusterer.predict(new_data)
numpy.savetxt("normalized_cluster_assignments.csv", a, delimiter=",")
